...
**التاسك الحالي:** Phase 1 مكتمل (عدا Exit Test)؛ **Phase 3 — T-301/T-303/T-305/T-309/T-310 مُنجَزة** (مراقب الخوادم + Docker، الإصدارات 1.30.0–1.33.0). التالي المتاح: T-202 Quake / T-204 Snippets / T-302 Security Suite
...
| ID | التاسك | الأولوية | الحالة | الجلسة | ملاحظات |
|---|---|---|---|---|---|
| T-301 | SSH Manager | P0 | ✅ |  | مُنجَز: «مراقب الخوادم» — بروفايلات خوادم SSH (host/port/user، مصادقة كلمة مرور/مفتاح PEM، أسرار DPAPI) في `Terminal.Storage/ServerProfileStore` + اتّصال `Terminal.Servers/Ssh/SshNetConnection` (SSH.NET، إعادة اتّصال تلقائيّة + keep-alive) + نافذة `Views/ServerMonitorWindow` (قائمة/محرّر/اتّصال/بحث موسّع بأسماء الحاويات). خيار sudo لأوامر Docker |
| T-302 | Security Suite | P0 | ⬜ |  | جزئيّ: تعمية أسرار الخوادم بـ DPAPI (`Services/SecretProtector`) منجَزة + حماية اللصق الخطر (T-104)؛ بقيّة الحزمة (تدقيق/سياسات) لم تبدأ |
| T-303 | SFTP | P1 | ✅ |  | مُنجَز: `Terminal.Servers/Ssh/SshNetSftp` (تنزيل/رفع) + تصفّح ملفّات الخادم (df/du/find) وعمليّات (حذف/تسمية/mkdir/tail) + تنزيل ملفّ من المضيف؛ ورفع/تنزيل ملفّات **داخل الحاويات** (docker cp + SFTP لمؤقّت) |
| T-304 | Port Forwarding | P2 | ⬜ |  | |
| T-305 | Docker Panel | P1 | ✅ |  | مُنجَز (موسّع): تعريف حاوية مالكة لملفّ (`DockerInspector`) + قائمة كلّ الحاويات (`DockerContainers`، بحث + تسمية مخصّصة مخزّنة لكلّ خادم `ContainerNameStore`) + **مستكشف حاوية** (`ContainerExplorerWindow`): تصفّح/عرض/تعديل/حذف/تسمية/رفع/تنزيل/mkdir داخل الحاوية (`ContainerFiles`) + **كونسول متعدّد التبويبات** عبر `docker exec` على اتّصال SSH مُصادَق (`SshShellSession : IPtySession`)، أسماء تبويبات + اختيار صدفة (bash/sh/مخصّص) + فصل/إرجاع + تخطيط أفقيّ/عموديّ. (1.30.0–1.33.0) |
| T-306 | Git Panel | P1 | ⬜ |  | |
| T-307 | DB Client | P2 | ⬜ |  | |
| T-308 | أدوات الشبكة | P2 | ⬜ |  | |
| T-309 | System Monitor | P2 | ✅ |  | مُنجَز: لوحة قيادة `ServerMonitorWindow` — CPU/RAM/Load/Uptime + أعلى العمليّات (`PerfMonitor`) + مراقبة حيّة + خريطة تخزين + أكبر الملفّات + تبويب إدارة (kill عمليّات، خدمات systemd، منافذ `ServerAdmin`) |
| T-310 | Logs Viewer | P2 | ✅ |  | مُنجَز: عارض سجلّ (آخر N سطر عبر `tail` + بحث تدريجيّ مع التفاف) في `ServerMonitorWindow`؛ وعارض محتوى ملفّ داخل الحاوية (`head -c` + كشف الثنائيّ) |
...
| 2026-07-14 | **Phase 3 — مراقب الخوادم + Docker مُنجَزة (T-301/303/305/309/310)** | طبقة `Terminal.Servers` (SSH.NET، محلّلات df/du/ps/ss، DockerInspector/Containers/Files) + `Terminal.Storage/ServerProfileStore`+`ContainerNameStore` + `Views/ServerMonitorWindow`+`ContainerExplorerWindow`. سُلّمت عبر الإصدارات 1.30.0–1.33.0 (سجلّها في `CHANGELOG.md`). اختبارات: 72 خوادم + 58 تخزين ✅ |
| 2026-07-14 | **كونسول الحاوية عبر SSH.NET ShellStream لا `ssh.exe`** | `SshShellSession` يحقّق `Terminal.Core.Pty.IPtySession` ويشغّل `docker exec -it` على **اتّصال SSH المُصادَق نفسه** → لا يُطلب كلمة مرور ثانية، بلا اعتماد على عميل OpenSSH محلّيّ. صار `TerminalTabView` يقبل `sessionFactory` (backend قابل للتبديل: ConPTY أو SSH). `Terminal.Servers` صار يرجع `Terminal.Core` لهذا الغرض |
| 2026-07-14 | **كتابة داخل الحاويات: docker exec + docker cp** | حذف/تسمية/mkdir عبر `docker exec` (rm/mv/mkdir، اقتباس مضيف طبقة واحدة)؛ الرفع/الحفظ عبر SFTP لمؤقّت على المضيف ثمّ `docker cp` (آمن للثنائيّ وأيّ حجم). أسماء الحاويات المخصّصة تُخزَّن بمفتاح (معرّف البروفايل + اسم الحاوية) فلا تتداخل الخوادم |
| 2026-07-14 | استخراج نموذج عنصر قائمة السياق عبر `PlacementTarget` لا `DataContext` | DataContext لعناصر `ContextMenu` داخل قوالب `ItemsControl` غير موثوق في WPF (كان يُفشِل تسمية الحاويات وبعض عمليّات المستكشف) — الحلّ الموثوق: `MenuItem → ContextMenu.PlacementTarget → DataContext` |

## 🗂️ أرشيف تحسينات مراقب الخوادم / Docker — طلبات المستخدم (جلسة 2026-07-14)
> سجلّ مفصّل بكلّ التحسينات المطلوبة، مُبوّبة بالإصدار الذي سُلّمت فيه. المرجع الرسميّ للبنود: [`../CHANGELOG.md`](../CHANGELOG.md).

### الإصدار 1.30.0 — دخول الحاوية (الأساس)
- **شِل تفاعليّ كامل داخل حاوية Docker** عبر SSH (تنقّل + تنفيذ أوامر بأدوات الحاوية).
- **تبويب «الحاويات»**: قائمة `docker ps -a` بالأسماء/الصور/الحالة + زرّ «دخول» للشغّالة.
- **دعم sudo** لأوامر Docker (خيار في بروفايل الخادم).

### الإصدار 1.31.0 — المستكشف + التسمية + البحث
- **مستكشف الحاوية**: تصفّح ملفّات + كونسول مدمج، مع «الكونسول هنا» (cd).
- **عرض محتوى الملفّ** (head -c 512KB، كشف الثنائيّ/الاقتطاع، نسخ).
- **تسمية الحاويات** بأسماء مخصّصة مخزّنة لكلّ خادم + **بحث الخوادم** بالعنوان/اسم الحاوية.
- إخفاء/إظهار الكونسول؛ **إصلاح**: نافذة المراقب صارت مستقلّة (لا تبقى فوق).

### الإصدار 1.32.0 — إدارة ملفّات الحاوية
- **حذف** ملفّ/مجلّد (تأكيد، rm -rf للمجلّد) و**إعادة تسمية** (mv) — قائمة سياق.
- **مجلّد جديد** (mkdir) و**رفع ملفّ** من الجهاز (SFTP → docker cp).
- **تعديل وحفظ ملفّ نصّي** في العارض. **الكونسول هنا** على مجلّد.

### الإصدار 1.33.0 — كونسول متعدّد + إصلاحات
- **كونسول متعدّد التبويبات** داخل الحاوية: «+» تيرمنال جديد، أسماء تبويبات، **اختيار الصدفة** (Bash/Sh/مخصّص)، فصل/إرجاع لكلّ تبويب، تخطيط أفقيّ/عموديّ، تكبير/طيّ.
- **بحث في قائمة الحاويات** (بالاسم/الاسم المخصّص/الصورة).
- **إصلاحات**: تسمية الحاويات (PlacementTarget)، تشغيل التيرمنال من مجلّد عمل محدّد (توسيع/تطبيع المسار)، تجمّد المراقب عند إغلاق المستكشف (تخلّص خلفيّ)، شريط تمرير السجلّ (ثيم)، بحث سريع + تنقّل لوحة مفاتيح في سجلّ الأوامر، ✕ الكونسول يطويه لا يغلق النافذة.
